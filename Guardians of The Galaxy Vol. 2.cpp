...
float collectibleSize = 0.05f; //0.4f;
...
void drawCollectible(float x, float z) {
	...
	glRotated(collectibleRotation, 0, 1, 0);
...
bool checkCollectibleCollision(float shipX, float shipZ, float collectibleX, float collectibleZ) {
	...
	if (fabs(shipX - collectibleX) < (shipSize + collectibleSize) / 2.0 + 0.5 &&
		fabs(shipZ - collectibleZ) < (shipSize + collectibleSize) / 2.0 + 0.5) {
		return true;
	}
...
void updateCollectibleRotation(int value) {

	collectibleRotation += 5.0f;

	glutPostRedisplay();

	glutTimerFunc(50, updateCollectibleRotation, 0); // 50 milliseconds (about 20 frames per second)
}
...
bool checkObstacleCollision(float shipX, float shipZ, float obstacleX, float obstacleZ) {
	...
	if (fabs(shipX - obstacleX) < (shipSize + obstacleSize) / 2.0 + 0.5 &&
		fabs(shipZ - obstacleZ) < (shipSize + obstacleSize) / 2.0 + 0.5 ) {
		return true;
	}
...
void updateObstacleRotation(int value) {

	obstacleRotation += 5.0f;

	glutPostRedisplay();

	glutTimerFunc(50, updateObstacleRotation, 0); // 50 milliseconds (about 20 frames per second)
}
...
void updatePlanetRotation(int value) {

	planetRotation += 0.5f;

	glutPostRedisplay();

	glutTimerFunc(50, updatePlanetRotation, 0); // 50 milliseconds (about 20 frames per second)
}
...
void updateGameTimer(int value) {
	...
	glutTimerFunc(1000, updateGameTimer, 0); // Set up the next timer call (e.g., 1000 milliseconds for 1 second)
}
...
void Display() {
	...
	if (sceneOne) {
		...
		if (isGameOver || health <= 0) {
			...
			glPushMatrix();
			glLoadIdentity();
			gluOrtho2D(0, 640, 0, 480); // window dimensions
			...
			glRasterPos2f(640 / 2.0 - 50, 480.0 / 2); 
		...
		if (checkPlanetCollision && isGameOver && health != 0 && remainingTime != 0 && score != 0) {
			...
			glRasterPos2f(640 / 2.0 - 120, 480.0 / 2); 
...
void Keyboard(unsigned char key, int x, int y) {
	...
	case '1': // Switch to scene & one only done for testing quickly
		sceneOne = true;
		sceneTwo = false;
		break;

	case '2': // Switch to scene two & only done for testing quickly
		sceneOne = false;
		sceneTwo = true;
		break;
...
void main(int argc, char** argv) {
	...
	// Set up the initial timer call for collectible rotation
	glutTimerFunc(50, updateCollectibleRotation, 0);

	// Set up the initial timer call for obstacle rotation
	glutTimerFunc(50, updateObstacleRotation, 0);

	// Set up the initial timer call for planet rotation
	glutTimerFunc(50, updatePlanetRotation, 0);
